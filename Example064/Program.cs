...
// for (int u = 1; u < pascal.GetLength(1); u++)
// {
//     if (pascal[(pascal.GetLength(0) - 2), u] > max) {max = pascal[(pascal.GetLength(0) - 2), u];}
// }
// temp = Convert.ToString(max);
// max = temp.Length;
// for (int i = (pascal.GetLength(0) - 1); i >= 0; i--)
// {
//     for (int j = 0; j < pascal.GetLength(1); j++)
//     {
//         if (i == (pascal.GetLength(0) - 1))
//         {
//             if (pascal[i, j] != 0) {pascalTriangle[i, j] = Convert.ToString(pascal[i, j]);}
//             else 
//             {
//                 for (int g = 0; g < max; g++)
//                 {
...
//         else
//         {
//             if (pascal[(i + 1), j] == 0) {num = max;}
//             else {temp = Convert.ToString(pascal[(i + 1), j]); num = temp.Length;}
//             temp = Convert.ToString(pascal[i, j]);
//             num = num - (temp.Length);
//             for (int k = 0; k < num; k++)
//             {
//                 pascalTriangle[i, j] = pascalTriangle[i, j] + char1;
//             }
//             if (pascal[i, j] != 0) {pascalTriangle[i, j] = pascalTriangle[i, j] + Convert.ToString(pascal[i, j]);}
//         }
//     }
// }